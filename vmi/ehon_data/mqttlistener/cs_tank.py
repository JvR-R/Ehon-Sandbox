...
# Set up logging
log_file = "cs_Tank.log"  # Adjust the path as needed

# Configure the logger
logger = logging.getLogger("cs_Tank_logger")
...
def main():
    ...
    try:
        ...
        if connection.is_connected():
            ...
            crc_query = "SELECT crc_tank FROM console WHERE uid = %s;"
            ...
            date_query = """
            SELECT MAX(updated_at) FROM Tanks 
            WHERE uid = %s;
            """
            ...
            # Use the provided query to fetch tank_id, capacity, product_name
            check_query = """
            SELECT ts.tank_id, ts.capacity, ps.product_name
            FROM Tanks ts
            JOIN products ps ON ts.product_id = ps.product_id
            WHERE ts.uid = %s
            """
            ...
            if results:
                # Create a list to hold all tank data
                tanks_data = []
                for row in results:
                    tank_id = row[0]
                    capacity = row[1]
                    ...
                    row_values = [
                        str(tank_id),
                        str(capacity if capacity is not None else 0),
                        str(product_name if product_name is not None else ''),
                    ]
                    row_string = ','.join(row_values)
                    tanks_data.append(row_string)

                # Join the rows with '\n' to create the 'tanks' string
                tanks_list = '\n'.join(tanks_data) + '\n'
