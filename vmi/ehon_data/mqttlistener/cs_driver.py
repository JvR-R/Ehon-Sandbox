...
# Set up logging
log_file = "cs_driver.log"  # Adjust the path as needed

# Configure the logger
logger = logging.getLogger("cs_driver_logger")
...
def main():
    ...
    try:
        ...
        if connection.is_connected():
            ...
            crc_query = "SELECT crc_driver FROM console WHERE uid = %s;"
            ...
            date_query = """
            SELECT MAX(updated_at) FROM drivers 
            ...
            # Use the new query to fetch driver_id, first_name, surname
            check_query = """
            SELECT driver_id, first_name, surname FROM drivers 
            WHERE client_id IN (SELECT Client_id FROM Console_Asociation WHERE uid = %s);
            """
            ...
            if results:
                # Create a list to hold all driver data
                drivers_data = []
                for row in results:
                    driver_id, first_name, surname = row
                    driver_id = int(driver_id) if isinstance(driver_id, Decimal) else driver_id
                    first_name = str(first_name)
                    surname = '' if surname is None else str(surname)
                    
                    # Combine first_name and surname into driver_name
                    driver_name = f"{first_name} {surname}" if surname else first_name

                    row_values = [
                        int(driver_id),
                        driver_name  # No need to convert to string again
                    ]
                    row_string = ','.join(map(str, row_values))
                    drivers_data.append(row_string)
