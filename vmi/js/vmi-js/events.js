...
on('click', '.button-js3', async function () {
  ...
  // Validate alarms against [0..capacity]
  const cap = Math.max(0, Number(String(capacity).replace(/,/g, '')) || 0);
  const hh = Number(val(`#chigha-${rowIndex}`) || 0);
  const h  = Number(val(`#higha-${rowIndex}`)  || 0);
  const l  = Number(val(`#lowa-${rowIndex}`)   || 0);
  const ll = Number(val(`#clowa-${rowIndex}`)  || 0);
  if (cap > 0 && (h > cap || hh > cap)) {
  ...
  const data = {
    case  : 3,
    uid, tank_no, site_id, client_id, capacity: cap,
    higha   : h,
    lowa    : l,
    chigha  : hh,
    clowa   : ll,
