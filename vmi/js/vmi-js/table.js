...
export async function initTable() {
    ...
    columns : [
      ...
      { data:null, className:'hide-on-mobile', render:buildStatusIcon, orderable:true },
      ...
      rowCallback : (row,d)=>{
        ...
        row.dataset.cs_type    = 
          d.device_type == 201 ? 'MCS_PRO'      :
          d.device_type == 200 ? 'MCS_LITE'     :
          d.device_type == 30  ? 'EHON_GATEWAY' :
          d.device_type == 20  ? 'EHON_Link' :
          '';
        row.dataset.tankDeviceId = d.tank_device_id ?? '';
