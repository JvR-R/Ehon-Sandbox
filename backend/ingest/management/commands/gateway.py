"""
Unified MQTT listener for “gateway/#” topics
Place this file at ingest/management/commands/gateway.py
Run with:  python manage.py gateway --config /home/javier/config.ini
"""
...
# All device-specific handlers live in ingest/gateway/*.py
from ingest.gateway import HANDLERS   # HANDLERS = {filter: function, …}

# ───────────────────────────────────────────────────────────────────────────────
LOG  = logging.getLogger("gateway")              # main logger for this service
ERR  = logging.getLogger("gateway.err")          # error channel
TXLOG = logging.getLogger("gateway.transactions")# handler-specific logger name used by transactions.py

# --- file logging: /var/log/gateway/{gateway.log,gateway.err}
_LOG_BASE = Path("/var/log/gateway")
...
TXLOG.setLevel(logging.WARNING)  # let handler errors bubble into gateway.err

_fmt = logging.Formatter("%(asctime)s [%(levelname)s] %(message)s")
_fh_main = RotatingFileHandler(_LOG_BASE / "gateway.log", maxBytes=10*1024*1024, backupCount=5, encoding="utf-8")
_fh_main.setFormatter(_fmt)
LOG.addHandler(_fh_main)

_fh_err  = RotatingFileHandler(_LOG_BASE / "gateway.err",  maxBytes=10*1024*1024, backupCount=5, encoding="utf-8")
_fh_err.setFormatter(_fmt)
ERR.addHandler(_fh_err)
TXLOG.addHandler(_fh_err)        # IMPORTANT: make transactions go to gateway.err
...
# Per-device rotating logs under management/logs/gateway/<MAC>.log (2MB per file)
LOG_DIR = (Path(__file__).resolve().parent.parent / "logs" / "gateway")
...
def get_device_logger(device_id: str) -> logging.Logger | None:
    ...
    management/logs/gateway/<ID>.log with size-based rotation (1MB, 5 backups).
...
def load_cfg(path: str) -> dict:
    """Read [mqttgateway] keys from an INI file."""
...
def dispatch(topic: str, payload: dict, client: mqtt.Client) -> None:
    ...
    # Ignore bare device root topics like "gateway/<id>" (no subtopic)
    parts_for_root_check = topic.split("/")
    if len(parts_for_root_check) == 2 and parts_for_root_check[0] == "gateway":
        return
    for flt, func in HANDLERS.items():
        if mqtt.topic_matches_sub(flt, topic):
            parts  = topic.split("/")
            if len(parts) >= 3:                 # gateway/<id>/...
                serial = parts[1]
            else:                               # gateway/...
            ...
            # Example: gateway/<serial>/probes/<num> with payload {"conn": false}
...
def bump_last_conn(serial: str | None) -> None:
    ...
    serial is the <id> segment from 'gateway/<id>/(...)'.
    """
    if not serial:
        return
...
class Command(BaseCommand):
    help = "Listen on gateway/# and save incoming MQTT messages"

    def add_arguments(self, parser):
        parser.add_argument(
            "--config",
            required=True,
            help="Path to INI file that contains [mqttgateway] section",
        )
    ...
    def handle(self, *args, **opts):
        ...
        cli = mqtt.Client(
            client_id=cfg.get("client_name", "backend_gateway_listener"),
        ...
        def on_connect(c, u, f, rc):
            LOG.info("Connected (RC=%s); subscribing to gateway/#", rc)
            c.subscribe("gateway/#", qos=0)
